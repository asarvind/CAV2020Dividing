...
\includegraphics[scale = 0.39]{autocarImages/leg1.png}~
  ...
  \caption{Flowpipe bounds at different time points for
    car model \is{The font of the text above the plot for $x_1$ is smaller than the others. Could we place the legends a bit logically? May be IOE methods in the first row and the other methods in the second row?}}\label{fig:flowcar}
\end{figure}
% \begin{figure}
...
\emph{Results:}  The upper and lower bounds of flowpipe for different
  ...
  \is{We can order the subfigures better: $x_1$, $x_2$, steering angle, slip angle, yaw angle, yaw rate (I have made this change). Also, it may be worth mentioing why we are not plotting one state: heading velocity}.
...
\end{center}
\end{table}
\is{Should we change the older of the columns to $\eta = 3$, $\eta = 4$, $\eta = 5$ in Table 1? Computation time of the union method?}
%
\begin{figure}
%\vspace{-1.05em}
...
\emph{Results:}  Our IoU method for each of the $8$, $16$ and $32$ divisions per
  ...
  consequently infinite bounds $[-1,1]^{12}$ \is{How is it an infinite bound?} after $0.5\si{\second}$ simulation
  time, where the normalization error is taken to be $\rho
  = \taylor{f}{X_0}{u}$ \is{This has been mentioned before.}.  We plotted the flowpipe bounds for the
  ...
  to other methods. 
  \is{The lines describing the failure of the union method can be placed her.}
  The computation times are given in
  Table~\ref{tab:comptimes}.  The preprocessing time before simulation
  for our IoU method is $44\si{\second}$.
  \is{Should we mention what is done in the preprocessing step?}
%
