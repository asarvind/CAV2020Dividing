...
\set{x\in\reals\st a\leq x\leq b}$, $[-\infty,a] = \set{x\in\reals\st
  ...
  interval arithmetic to correctly bound the image of $f$ over $a$ \is{u?},
...
matrix $X$ \is{$\bar{X}$} is said to be bounded if $\forall
...
interval vector $u$ \is{$u \in \intervals$?} along its diagonal is denoted $\diag{u}$.  If
