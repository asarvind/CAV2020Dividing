\paragraph{Basic notation}:  The set of real numbers is $\reals$, rationals numbers is $\rationals$, 3
...
\begin{enumerate}
\item If $f = c$ for $c\in\rationals$, then $f(x) = c$.
...
\is{Should we use a different symbol than $\bowtie$ here? It has been used before.}
...
%
\begin{itemize}
\item $\forall i\in\set{1,\ldots,n}$, $\dom{f}\subseteq \dom{g}$. \is{What is $g$ here?}
...
a\iv{f}{x}$. \is{typo: $g$ instated of $f$.}
